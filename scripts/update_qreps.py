...
def main():
    ...
    templates = glob.glob(args.query_dir + "/*")

    qnames = None
    if os.path.exists("unique_plan_keys.csv"):
        qnames = pd.read_csv("unique_plan_keys.csv")
        qnames = qnames["qname"].values
        print(len(qnames))
        pdb.set_trace()

    for template in templates:
        print(template)
        fns = list(glob.glob(template + "/*"))
        qreps = []
        ...
        for i, fn in enumerate(fns):
            ...
            if "pkl" not in fn:
                qreps.append(None)
                continue
            curqname = os.path.basename(fn)

            qrep = load_sql_rep(fn)
            text = qrep["sql"]
            newdir = os.path.join(template, "sqls")
            make_dir(newdir)
            newfn = os.path.join(newdir, curqname + ".sql")
            ...
            for subset, info in qrep["subset_graph"].nodes().items():
                ...
                if not "actual" in cards:
                    ...
                    # if cards["actual"] == cards["expected"] \
                            # and cards["actual"] != 1 \
                            # and len(subset) > 1:
                    ...
                    # if len(subset) == 1 and \
                            # cards["expected"] > cards["actual"]*10 \
                            # and cards["actual"] < 10:
                        # sg = qrep["join_graph"].subgraph(subset)
                        # subsql = nx_graph_to_query(sg)
